...
export default function DuplicateProject(props: DuplicateProjectProps) {
  ...
  const theProject = createQuery(
    ...
    },
    {
      get enabled() {
        const u = user();
        return !u.isLoading && u.isAuthenticated && u.token !== null && organizations.isSuccess;
      },
      refetchOnWindowFocus: false,
      retryOnMount: false,
    }
  ...
  createEffect(() => {
    if (theProject.isSuccess && theProject.data && organizations.isSuccess && organizations.data) {
      setProject({
        name: `${theProject.data.name.split("/")[1]}-copy`,
        description: theProject.data.description ?? "",
        protected: "",
        visibility: theProject.data.visibility,
        org: theProject.data.name.split("/")[0] ?? Object.keys(organizations.data ?? {})[0] ?? "",
      });
    }
  });
        ...
        <Tabs.Content value="project" class="flex flex-col gap-2.5 w-full">
          ...
          >
            <Show
              ...
              fallback={
                <Switch>
                  <Match when={organizations.isLoading || theProject.isLoading}>

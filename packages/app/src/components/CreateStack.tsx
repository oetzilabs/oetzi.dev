...
import { Mutations, createStack } from "../utils/api/mutations";
        ...
        <div class="flex flex-col gap-2.5 w-full pb-8 border-b border-neutral-200 dark:border-neutral-800">
          ...
          >
            ...
            <TextField.TextArea
              ...
              class="p-2 py-1 w-full bg-neutral-50 dark:bg-neutral-950 rounded-md border border-neutral-200 dark:border-neutral-800 min-h-[150px]"
            />
          </TextField.Root>
          <TextField.Root
            class="w-full flex flex-col gap-0.5"
        ...
        <div class="flex flex-col gap-2.5 w-full pb-8 border-b border-neutral-200 dark:border-neutral-800">
          <RadioGroup.Root
            ...
            onChange={(x) => {
              if (!x) return;
              if (x === "url") {
                setSetupFrom({ type: "url", url: "", checked: false });
                checkStackFromUrl.reset();
              }
              if (x === "toml") {
                setSetupFrom({ type: "toml", file: null, checked: false });
                checkStackFromFile.reset();
              }
              if (x === "custom") {
                setSetupFrom({ type: "custom", checked: false });
              }
            }}
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        reader.onload = async (e) => {
                          ...
                          if (!text) return;
                          const fileResult = await checkStackFromFile.mutateAsync(text.toString());
